...
public class InMemoryTaskManagerTest {
    ...
    @Test
    void shouldCreateTaskWithTimeFields() {
        TaskManager taskManager = new InMemoryTaskManager();
        Task task = new Task("Задача", "Описание");
        task.setStartDateTime(LocalDateTime.of(2025, 4, 30, 12, 0));
        task.setDuration(Duration.ofMinutes(90));

        taskManager.createTask(task); // например, addTask(Task task)

        Task savedTask = taskManager.getTask(task.getId());
        assertEquals(task.getStartDateTime(), savedTask.getStartDateTime());
        assertEquals(task.getDuration(), savedTask.getDuration());
        assertEquals(task.getEndTime(), savedTask.getEndTime());
    }

    @Test
    void shouldCalculateEndTimeCorrectly() {
        Task task = new Task("Test", "With time");
        task.setStartDateTime(LocalDateTime.of(2025, 5, 1, 10, 0));
        task.setDuration(Duration.ofMinutes(120)); // 2 часа

        assertEquals(LocalDateTime.of(2025, 5, 1, 12, 0), task.getEndTime());
    }
